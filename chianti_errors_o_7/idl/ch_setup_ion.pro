...
FUNCTION ch_setup_ion, ions, wvlmin=wvlmin, wvlmax=wvlmax, ioneq_file=ioneq_file, $
                       ...
                       no_rrec=no_rrec,  add_uncertainties=add_uncertainties,$
              default_uncertainty=default_uncertainty, normal=normal, $
uncertainty=uncertainty,id_unc=id_unc
...
; 
;       v.10, 10-Jul-2019, Giulio Del Zanna (GDZ)
...
;             used (by default it is set to 50%). 
...
default_uncertainty=50


 ind2 = where(abs(wgfastr.wvl) NE 0., nnl)
        if nnl eq 0 then message, 'Error ! no lines ??! '
         
         if keyword_set(normal) then rnd=randomn(seed, nnl) else $
         rnd = -1.+2*randomu(seed, nnl)
 
openw, 1, 'data_rnd/rnd_a'+trim(id_unc)
printf,1, rnd
close,1

;save,file='rnd_a'+trim(id_unc)+'.save',rnd, /compress


if not file_exist(fname+'.a_unc') then $
         message,'Error, unc file not found !' else begin 
         
         da=read_ascii(fname+'.a_unc')
...
a_value[lvl1[ind2[ii]]-1,lvl2[ind2[ii]]-1]=a_value[lvl1[ind2[ii]]-1,lvl2[ind2[ii]]-1]*$
       ((1. + unc/100.*rnd[ii]) > 0.01) 
...
openw, 1, 'data_rnd/rnd_ups'+trim(id_unc)
printf,1, rnd
close,1
     
; save,file='rnd_ups'+trim(id_unc)+'.save',rnd, /compress

            if n_elements(uncertainty) eq 0 then begin 
; read the uncertainty file. search in the working directory.
      
      if not file_exist(fname+'.ups_unc') then $
         message,'Error,  unc ups  file not found !' else begin 
         
         da=read_ascii(fname+'.ups_unc')
         lower=fix(reform(da.field1[0,*]))
         upper=fix(reform(da.field1[1,*]))
         perc=reform(da.field1[2,*])
...
; check if the uncertainty is present, otherwise use the  default
         ...
         for ii=0L,splstr.info.ntrans-1 do begin 
            ...
            if n_good gt 0 then splstr.data[ii].spl[good] = $
               splstr.data[ii].spl[good]*((1. + uncertainty/100.*rnd[ii]) > 0.01 )
