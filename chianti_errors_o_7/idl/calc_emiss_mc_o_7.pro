...
pro calc_emiss_mc_o_7,  nsim=nsim, normal=normal, $
                   out_file=out_file, uncertainty=uncertainty

sngl_ion='o_7'

;wvl_list=[17.768, 18.627, 21.602, 21.805,  22.101]
wvl_list=[17.396, 17.768, 18.627, 18.672,21.602, 21.805,  22.101]
...
if n_elements(dl) eq 0 then dl=0.003


; --- number of realizations to be run,

if n_elements(nsim) eq 0 then nsim=10000
...
; extend to 8 
logt=5.8+ 0.1* indgen((8-5.8)/0.1+2)  
n_logt=n_elements(logt)
...
data = {wvl: 0.0, trans: '', em: dblarr(nsim, n_logt, nlogn)}
...
; store the unperturbed as the  first one    
   if k eq 0 then emiss = emiss_calc(iz, ion, temp=logt, dens=logn, ioneq_file=ioneq_file, /quiet,$
           path='.', /noionrec,/no_rrec) $
                          else $   
                             emiss = emiss_calc(iz, ion, temp=logt, dens=logn, ioneq_file=ioneq_file, /quiet,$
                                               uncertainty=uncertainty, /add_uncertainties,id_unc=k, normal=normal,$
  path='.', /noionrec,/no_rrec)
   ...
   for i=0, n_wvl-1 do begin
      ...
      if n_nearby eq 1 then em=reform(emiss[match].em) else if n_nearby gt 1 then $
         em=total(reform(emiss[match[*]].em),3) else if  n_nearby eq 0 then $
...
if n_elements(uncertainty) gt 0 then out_file=out_file+'_'+trim(uncertainty)+'%'

save, data,logn, logt, file=out_file+'_nsim='+trim(nsim)+'.save',/compress
