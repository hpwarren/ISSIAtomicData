...
;; ##########################################################################################
;; Modifications by HPW to introduce white noise perturbations to the A values.

  CHIANTI_PERTURB_AVAL = float(getenv('CHIANTI_PERTURB_AVAL'))

  if keyword_set(CHIANTI_PERTURB_AVAL) then begin
    print, ' | Perturbing the A values '
    print, ' |      A value sigma = '+trim(CHIANTI_PERTURB_AVAL,'(f10.2)')
    
    perturb = 1 + CHIANTI_PERTURB_AVAL*randomn(seed, nindex)
    a_value = a_value*perturb
  endif

;; End of modifications by HPW
;; ##########################################################################################  
